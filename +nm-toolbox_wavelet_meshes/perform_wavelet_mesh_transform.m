...
if size(f,2)>1
    for i=1:size(f,2)
        f(:,i) = nm-toolbox_wavelet_meshes.perform_wavelet_mesh_transform(vertex,face, f(:,i), dir, options);
    end
    return;
end
...
do_update = nm-toolbox_general.getoptions(options,'do_update', 1);
do_scaling = nm-toolbox_general.getoptions(options,'do_scaling', 1);
...
if do_update
    ...
    for j=J-1:-1:1
        ...
        vring = nm-toolbox_graph.compute_vertex_ring(face{j+1});
        e2f = nm-toolbox_graph.compute_edge_face_ring(face{j});
        fring = nm-toolbox_graph.compute_face_ring(face{j});
        facej = face{j};
        ...
        for k=nj+1:nj1  % for all the fine point
            ...
            [e,v,g] = nm-toolbox_wavelet_meshes.compute_butterfly_neighbors(k, nj);
...
for j=jlist
    ...
    if dir==-1 && do_update        
        for k=nj+1:nj1  % for all the fine point
            % retrieve coarse neighbors
            e = nm-toolbox_wavelet_meshes.compute_butterfly_neighbors(k, nj);
