...
sub_type = nm-toolbox_general.getoptions(options, 'sub_type', '1:4');
spherical = nm-toolbox_general.getoptions(options, 'spherical', 0);
...
if nsub>1
    ...
    face1 = face;
    for i = 1:nsub
         [f1,face1] = nm-toolbox_wavelet_meshes.perform_mesh_subdivision(f1,face1,1, options);
    end
    return;    
end
...
verb = nm-toolbox_general.getoptions(options, 'verb', n>500);
loop_weigths = nm-toolbox_general.getoptions(options, 'loop_weigths', 1);
...
if topology==3
    ...
    switch interpolation
        ...
        case 'sqrt3'
            face1 = [];
            edge = nm-toolbox_graph.compute_edges(face);
            ne = size(edge,2);
            e2f = nm-toolbox_graph.compute_edge_face_ring(face);
            face1 = [];
            % create faces
            for i=1:ne
                if verb
                    nm-toolbox_general.progressbar(i,n+ne);
                end
            ...
            % move old vertices
            vring0 = nm-toolbox_graph.compute_vertex_ring(face);
            for k=1:n
                if verb
                    nm-toolbox_general.progressbar(k+ne,n+ne);
                end
                m = length(vring0{k});
    ...
    switch interpolation
        ...
        case 'butterfly'
            ...
            vring = nm-toolbox_graph.compute_vertex_ring(face1);
            e2f = nm-toolbox_graph.compute_edge_face_ring(face);
            fring = nm-toolbox_graph.compute_face_ring(face);
            facej = face;
            f1 = zeros(size(f,1),n+ne);
            f1(:,1:n) = f;
            for k=n+1:n+ne
                if verb
                    nm-toolbox_general.progressbar(k-n,ne);
                end
                [e,v,g] = nm-toolbox_wavelet_meshes.compute_butterfly_neighbors(k, n);
        ...
        case 'loop'
            ...
            for k=n+1:n+ne
                ...
                [e,v] = nm-toolbox_wavelet_meshes.compute_butterfly_neighbors(k, n);
