...
options.null = 0;
sampling_type = nm-toolbox_general.getoptions(options, 'sampling_type', 'area');
...
posw = nm-toolbox_wavelet_meshes.perform_spherial_planar_sampling(pos_sphere, sampling_type, options);
...
gim = nm-toolbox_general.rescale(gim);
...
remove_nan = nm-toolbox_general.getoptions(options, 'remove_nan', 1);
...
for i=1:nface
    if verb
        nm-toolbox_general.progressbar(i,nface);
    end
