...
options.null = 0;
base_mesh = nm-toolbox_general.getoptions(options, 'base_mesh', 'ico');
keep_subdivision = nm-toolbox_general.getoptions(options, 'keep_subdivision', 0);
relaxation = nm-toolbox_general.getoptions(options, 'relaxation', 0);
...
[vertex{1},face{1}] = nm-toolbox_wavelet_meshes.compute_base_mesh(base_mesh, 0, options);
for j=2:J
    ...
    if relaxation>0
        ...
        for i=1:relaxation
            vertex{end} = nm-toolbox_graph.perform_mesh_smoothing(face{end},vertex{end},vertex{end},options)';
