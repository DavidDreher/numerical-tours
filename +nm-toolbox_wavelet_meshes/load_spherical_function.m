...
switch name
    ...
    case 'singular'
        f = abs(x).^.4;
    case 'image'
        name = nm-toolbox_general.getoptions(options, 'image_name', 'lena');
        if isempty(M)
            M = nm-toolbox_signal.load_image(name);
            q = size(M,1);
            q = min(q,512);
            M = nm-toolbox_general.crop(M,q);
            M = nm-toolbox_signal.perform_blurring(M,4);
        end
    ...
    case {'earth' 'earth-nm-toolbox_signal.grad'}
        filename = 'earth-bw';
        M = double( nm-toolbox_signal.load_image(filename) );
        M = nm-toolbox_signal.perform_blurring(M,4);
        if strcmp(name, 'earth-nm-toolbox_signal.grad')
            G = nm-toolbox_signal.grad(M);
            M = sqrt(sum(G.^2,3));
            M = nm-toolbox_signal.perform_blurring(M,15);
        end
        f = perform_spherical_interpolation(pos,M,0);
end
