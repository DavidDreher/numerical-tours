...
if strcmp(type, 'mesh')
    ...
    n = 256;
    M = nm-toolbox_wavelet_meshes.perform_sgim_sampling(vertex, sphparam, face, n);
    Mnormal = nm-toolbox_wavelet_meshes.perform_sgim_sampling(normal, sphparam, face, n);
end

% save geometry image
nm-toolbox_wavelet_meshes.toolbox.write_gim(M, [rep name '-' type], 0);
nm-toolbox_wavelet_meshes.toolbox.write_gim(Mnormal, [rep name '-' type '.normal'], 0);

nm-toolbox_wavelet_meshes.plot_geometry_image(M);
colormap gray(256);
