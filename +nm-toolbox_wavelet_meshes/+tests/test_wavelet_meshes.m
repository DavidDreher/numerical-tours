...
if strcmp(name, 'sphere') || strcmp(name, 'triangle') || strcmp(name, 'rand')
    ...
    if strcmp(name, 'triangle') || strcmp(name, 'rand')
        ...
        f = nm-toolbox_wavelet_meshes.load_spherical_function(func, vertex{end}, options);
    end
...
if not(strcmp(name, 'sphere'))
    for j=Jmin:min(J,6)
        clf;
        if strcmp(name, 'triangle') || strcmp(name, 'rand')
            nm-toolbox_graph.plot_graph(nm-toolbox_graph.triangulation2adjacency(face{j}), vertex{j}(1:2,:));
        else
            nm-toolbox_graph.plot_mesh(vertex{j},face{j},options);
...
for i = 1:length(rlist)
    ...
    if not(strcmp(name, 'sphere'))
        ...
        nm-toolbox_graph.plot_mesh(f1,face{j},options);
    end
    if strcmp(name, 'triangle')
...
for j = [3 4 5]
    ...
    nm-toolbox_wavelet_meshes.plot_spherical_function(-vertex{end},face{end},f2, options);
