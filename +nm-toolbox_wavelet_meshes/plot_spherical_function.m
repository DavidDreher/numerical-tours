...
options.null = 0;
rho = nm-toolbox_general.getoptions(options, 'rho', .5);
color = nm-toolbox_general.getoptions(options, 'color', 'nm-toolbox_general.rescale');

use_color       = nm-toolbox_general.getoptions(options, 'use_color', 1);
use_elevation   = nm-toolbox_general.getoptions(options, 'use_elevation', 1);
...
if use_color
    % scale the colors
    switch color
        case 'nm-toolbox_general.rescale'
            fv = nm-toolbox_general.rescale(f);
        case 'wavelets'
            fv = f;
            if sum(fv<0)>0
...
if use_elevation
    ...
    if std(d)>.01 % we are not on a sphere
         normals = nm-toolbox_graph.compute_normal(vertex,face);
    end
...
nm-toolbox_graph.plot_mesh(vertex, face, options);
colormap jet(256);

func = nm-toolbox_general.getoptions(options, 'func', '');
switch func
    case {'image', 'earth'}
        image_name = nm-toolbox_general.getoptions(options, 'image_name', 'lena');
