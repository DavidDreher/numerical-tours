...
switch  lower(type)
    case 'rand'
        nverts_base = nm-toolbox_general.getoptions(options, 'nverts_base', 30);
    ...
    case 'square'
        nbr = nm-toolbox_general.getoptions(options, 'n', 2^j);
        x = 1:nbr;
        [Y,X] = meshgrid(x,x);
        I = X+(Y-1)*nbr;
        a = I(1:end-1, 1:end-1);
...
% subdivision
[vertex,face] = nm-toolbox_wavelet_meshes.perform_mesh_subdivision(vertex,face,j);
