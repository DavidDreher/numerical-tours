function M = nm-toolbox_signal.perform_blurring(M, sigma, options)
...
if size(M,3)>1
    for i=1:size(M,3)
        M(:,:,i) = nm-toolbox_wavelet_meshes.toolbox.nm-toolbox_signal.perform_blurring(M(:,:,i), sigma, options);
    end
    return;
end
