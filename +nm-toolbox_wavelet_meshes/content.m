...
for i=1:2
    f0(i,:) = nm-toolbox_general.rescale(f0(i,:),.01,.99);
end
...
clf;
for j=0:Jmax
    f = nm-toolbox_wavelet_meshes.perform_curve_subdivision(f0, j, options);
...
[vertex,face] = nm-toolbox_wavelet_meshes.compute_base_mesh('ico');
options.spherical = 1;
options.relaxation = 3;
clf;
for s=0:3
    subplot(2,2,s+1);
    nm-toolbox_graph.plot_mesh(vertex,face); axis tight;
...
name = 'mannequin';
vertex = {}; face = {};
[vertex{1},face{1}] = nm-toolbox_graph.read_mesh(name);
options.name = name;
...
for j=2:3
    if j>1
        [vertex{j},face{j}] = nm-toolbox_wavelet_meshes.perform_mesh_subdivision(vertex{j-1}, face{j-1}, 1, options);
    end
    subplot(1,2,j-1);
    hold on;
    nm-toolbox_graph.plot_mesh(vertex{j},face{j},options); axis tight;
    title([num2str(j) ' levels of subdivisons']);
    shading faceted;
...
options.use_color = 1;
options.rho = .3;
options.color = 'nm-toolbox_general.rescale';
...
f = nm-toolbox_wavelet_meshes.load_spherical_function('earth', vertex{end}, options);

%% 
% Perform the wavelet transform and remove small coefficients.

fw = nm-toolbox_wavelet_meshes.perform_wavelet_mesh_transform(vertex,face, f, +1, options);
% threshold (remove) most of the coefficient
r = .1;
fwT = nm-toolbox_wavelet_meshes.toolbox.keep_biggest( fw, round(r*length(fw)) );
% backward transform
f1 = nm-toolbox_wavelet_meshes.perform_wavelet_mesh_transform(vertex,face, fwT, -1, options);
% display
clf;
subplot(1,2,1);
nm-toolbox_wavelet_meshes.plot_spherical_function(vertex,face,f, options);
title('Original function');
subplot(1,2,2);
nm-toolbox_wavelet_meshes.plot_spherical_function(vertex,face,f1, options);
...
for j = [J-3 J-2]
    ...
    f2 = nm-toolbox_wavelet_meshes.perform_wavelet_mesh_transform(vertex,face, fw2, -1, options);
    ...
    subplot(1,2,i);
    nm-toolbox_wavelet_meshes.plot_spherical_function(-vertex{end},face{end},f2, options); axis tight;
...
for j=1:length(selj)
    subplot(2,2,j);
    nm-toolbox_graph.plot_mesh(vertex{selj(j)},face{selj(j)}, options);
...
clf;
subplot(1,2,1);
nm-toolbox_graph.plot_mesh(f,face{end},options); shading interp; axis tight;
title('Original surface');
subplot(1,2,2);
nm-toolbox_graph.plot_mesh(f1,face{end},options); shading interp; axis tight;
