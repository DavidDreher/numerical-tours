...
sub = nm-toolbox_general.getoptions(options, 'sub', round(size(H,1)/30) );
color = nm-toolbox_general.getoptions(options, 'color', 'r');
...
if size(H,3)==3 && size(H,4)==1
    ...
    [e1,e2,l1,l2] = nm-toolbox_signal.perform_tensor_decomp(H);
    H = nm-toolbox_signal.perform_tensor_recomp(e2,e1,l1,l2);
    end
    h = nm-toolbox_signal.plot_tensor_field(H, M, sub);
    return;
end
...
hold on;
if ~isempty(M)
    imagesc(nm-toolbox_general.rescale(M)); drawnow;
end
