...
switch lower(name)
    ...
    case 'stepregular'
        ...
        a = nm-toolbox_general.rescale(a,-0.1,0.1);
        y = y+a;
    case 'gaussiannoise'
        ...
        h = nm-toolbox_signal.compute_gaussian_filter(m,sigma/(4*n),n);
        options.bound = 'per';
        y = nm-toolbox_signal.perform_convolution(y,h, options);
    case 'rand'
        ...
        for i=1:n
            ...
            y(i) = nm-toolbox_general.rand_discr(p, 1);
        end
    case 'bird'
        [y,fs] = nm-toolbox_signal.load_sound([name '.wav'], n, options);
    case 'tiger'
        [y,fs] = nm-toolbox_signal.load_sound([name '.au'], n, options);
    case 'bell'
        [y,fs] = nm-toolbox_signal.load_sound([name '.wav'], n, options);
    otherwise
        y = MakeSignal(name,n);
...
elseif strcmp(Name,'hypchirps'), % Hyperbolic Chirps of Mallat's book
    ...
    enveloppe(M-P+1:M) = nm-toolbox_general.reverse(enveloppe(1:P));
    env 	= zeros(1,n);
...
elseif strcmp(Name,'chirps'), % Mixture of Chirps of Mallat's book
    ...
    t 	= (1:n)./n.*pi;
    f2 	= cos(a.*(t.^3));
    f2 	= nm-toolbox_general.reverse(f2);
    ix 	= (-n:n)./n.*20;
    g 	= exp(-ix.^2*4*n/1024);
    i1 	= (n/2+1:n/2+n);
    i2 	= (n/8+1:n/8+n);
    ...
    enveloppe(7*n/8+1:n) = nm-toolbox_general.reverse(enveloppe(1:n/8));
    sig 	= sig.*enveloppe;
...
elseif strcmp(Name,'cusp2'),
    ...
    sig(M-2.5*N+2:M-1.5.*N+1) = nm-toolbox_general.reverse(x);
