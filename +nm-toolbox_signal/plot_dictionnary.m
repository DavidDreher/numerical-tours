...
s = nm-toolbox_general.getoptions(options, 's', 1);
...
normalization = nm-toolbox_general.getoptions(options, 'normalization', 'nm-toolbox_general.rescale');

k = 0;
for i=1:nb(1)
    for j=1:nb(2)
        k = k+1;
        if k<=length(I) % nm-toolbox_general.rescale
            v = D(:,I(k));
            if strcmp(normalization, 'nm-toolbox_general.clamp')
                v = nm-toolbox_general.clamp( 3*v/vmax, -1,1 );
                v = (v+1)/2;
            else
                v = nm-toolbox_general.rescale(v);
            end
            v = reshape(v,w1,w1,s);
