...
if iscell(M)
    ...
    for i=1:q
        ...
        h{i} = nm-toolbox_signal.imageplot(M{i},str1, a,b,c-1+i);
    end
    global axlist;
...
elseif size(M,3)<=3
    ...
    h = imagesc(nm-toolbox_general.rescale(M)); axis image; axis off;
else
    ...
    a = nm-toolbox_graph.compute_alpha_map('gaussian', options); % you can plot(a) to see the alphamap

    % volumetric image
    h = nm-toolbox_graph.vol3d('cdata',nm-toolbox_general.rescale(M),'texture','2D');
    view(3);
    axis tight; % daspect([1 1 .4])
    colormap bone(256);
%    alphamap('rampup');
    % alphamap(.06 .* alphamap);
    nm-toolbox_graph.vol3d(h);
end
if not(isempty(str))
    title(str);
end
