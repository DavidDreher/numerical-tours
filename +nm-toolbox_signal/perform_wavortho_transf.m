...
options.null = 0;
h = nm-toolbox_general.getoptions(options,'h', nm-toolbox_signal.compute_wavelet_filter('Daubechies',4) );
...
if dir==1
    ...
    for j=Jmax:-1:Jmin
        sel = 1:2^(j+1);
        a = subselect(f,sel);
        for d=1:nm-toolbox_general.nb_dims(f)
            a = cat(d, nm-toolbox_signal.subsampling(nm-toolbox_signal.cconvol(a,h,d),d), nm-toolbox_signal.subsampling(nm-toolbox_signal.cconvol(a,g,d),d) );
        end
        f = subassign(f,sel,a);
    end
else
    ...
    for j=Jmin:Jmax
        ...
        for d=1:nm-toolbox_general.nb_dims(f)
            ...
            a = nm-toolbox_signal.cconvol(nm-toolbox_signal.upsampling(a,d),nm-toolbox_general.reverse(h),d) + nm-toolbox_signal.cconvol(nm-toolbox_signal.upsampling(w,d),nm-toolbox_general.reverse(g),d);
        end
        f = subassign(f,sel,a);
    end    
end
...
switch nm-toolbox_general.nb_dims(f)
    case 1
        f = f(sel);
    case 2
        f = f(sel,sel);
