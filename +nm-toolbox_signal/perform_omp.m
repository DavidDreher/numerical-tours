...
options.null = 0;
nbr_max_atoms = nm-toolbox_general.getoptions(options, 'nbr_max_atoms', 50);

tol = nm-toolbox_general.getoptions(options, 'tol', 1e-3);
use_slow_code = nm-toolbox_general.getoptions(options, 'use_slow_code', 0);
...
for k=1:1:P,
    if P>20 && verb==1
        nm-toolbox_general.progressbar(k,P);
    end
    a=[];
    x = Y(:,k);
    r = x; % residual
...
if size(f,2)>1
    ...
    for i=1:s
        if s>20
            nm-toolbox_general.progressbar(i,s);
        end
