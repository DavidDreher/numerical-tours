...
options.null = 0;
style = nm-toolbox_general.getoptions(options, 'style', 'real');
edge_color = nm-toolbox_general.getoptions(options, 'edge_color', 'r');
renormalize = nm-toolbox_general.getoptions(options, 'renormalize', 0);
lw = nm-toolbox_general.getoptions(options, 'line_width', 2);
gamma = nm-toolbox_general.getoptions(options, 'gamma', .8);
...
separable = nm-toolbox_general.getoptions(options, 'separable',0);
if separable
    A = M*0;
    for j1=[-1 Jmin:Jmax]
        sel1 = 2^j1+1:2^(j1+1);
        ...
        for j2=[-1 Jmin:Jmax]
            ...
            M1 = M(sel1,sel2,:);
            if j1<0 && j2<0
                M1 = nm-toolbox_general.rescale(M1);
            else
            vmax = max(abs(M1(:)));
            if vmax<eps
                vmax = 1;
            ...
            M1 = nm-toolbox_general.clamp(M1, -1,1);
            M1 = (M1+1)/2;
            end
            M(sel1,sel2,:) = M1; % nm-toolbox_general.rescale( M1 );
        end
    end
    hold on;
    s = [1/2 n-1/2];
...
for j=Jmin:Jmax
    qmin = double(~(j==Jmin));
    for q=qmin:3
        [selx,sely] = nm-toolbox_signal.compute_quadsel(j,q);
        M1 = M(selx,sely,:);
        if strcmp(style, 'abs')
            M1 = nm-toolbox_general.rescale( abs(M1) );
            if renormalize
                M1 = 1.5*M1;
                I = find(abs(M1)>1);
                M1(I) = M1(I)./abs(M1(I));
        ...
        elseif strcmp(style, 'real')
            if q>0
                ...
                M1 = 0.3 * M1/st;
                M1 = nm-toolbox_general.clamp(M1, -1,1);
                M1 = (M1+1)/2; % nm-toolbox_general.rescale( M1 );
            else
                M1 = nm-toolbox_general.rescale(M1);
            end
