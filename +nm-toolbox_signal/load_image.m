...
if iscell(type)
    for i=1:length(type)
        M{i} = nm-toolbox_signal.load_image(type{i},n,options);
    end
    return;
end
...
eta         = nm-toolbox_general.getoptions(options, 'eta', .1);
gamma       = nm-toolbox_general.getoptions(options, 'gamma', 1/sqrt(2));
radius      = nm-toolbox_general.getoptions(options, 'radius', 10);
center      = nm-toolbox_general.getoptions(options, 'center', [0 0]);
center1     = nm-toolbox_general.getoptions(options, 'center1', [0 0]);
w           = nm-toolbox_general.getoptions(options, 'tube_width', 0.06);
nb_points   = nm-toolbox_general.getoptions(options, 'nb_points', 9);
scaling     = nm-toolbox_general.getoptions(options, 'scaling', 1);
theta       = nm-toolbox_general.getoptions(options, 'theta', 30 * 2*pi/360);
eccentricity = nm-toolbox_general.getoptions(options, 'eccentricity', 1.3);
sigma = nm-toolbox_general.getoptions(options, 'sigma', 0);
...
switch lower(type)
    ...
    case 'bump'
        ...
        s = nm-toolbox_general.getoptions(options, 'bump_size', .5);
        c = nm-toolbox_general.getoptions(options, 'center', [0 0]);
        if length(s)==1
            s = [s s];
        end
        x = linspace(-1,1,n);
    ...
    case 'periodic'
        ...
        [Y,X] = meshgrid(x,x);
        f = nm-toolbox_general.getoptions(options, 'freq', 6);
    ...
    case 'ellipse-thin'
        ...
        M = nm-toolbox_signal.load_image('ellipse', n, options);
    case 'ellipse-fat'
    ...
    case 'square-tube-1'
        ...
        M = nm-toolbox_signal.load_image('square-tube', n, options);        
    case 'square-tube-2'
    ...
    case 'polygon'
        ...
        radius = scaling*nm-toolbox_general.rescale(rand(nb_points,1), 0.1, 0.93);        
    ...
    case 'polygon-8'
        options.nb_points = 8;
        M = nm-toolbox_signal.load_image('polygon', n, options);
    case 'polygon-10'
        options.nb_points = 8;
        M = nm-toolbox_signal.load_image('polygon', n, options);
    case 'polygon-12'
    ...
    case 'pacman'
        options.radius = 0.45;
        options.center = [.5 .5];
        M = nm-toolbox_signal.load_image('disk', n, options);        
        x = linspace(-1,1,n);
        [Y,X] = meshgrid(x,x);
        T =atan2(Y,X);
    ...
    case 'square-hole'
        ...
        M = nm-toolbox_signal.load_image('disk', n, options); 
        options.scaling = 0.5;
        M = M - nm-toolbox_signal.load_image('polygon-10', n, options); 
        
    case 'grid-circles'
        if isempty(n)
            n = 256;
        end
        f = nm-toolbox_general.getoptions(options, 'frequency', 30);
        eta = nm-toolbox_general.getoptions(options, 'width', .3);
    ...
    case 'chessboard'
        width = nm-toolbox_general.getoptions(options, 'width', round(n/16) );
    ...
    case 'squareregular'
        M = nm-toolbox_general.rescale(nm-toolbox_signal.load_image('square',n,options));
        if not(isfield(options, 'alpha'))
            options.alpha = 3;
        end
        S = nm-toolbox_signal.load_image('fnoise',n,options);
        M = M + nm-toolbox_general.rescale(S,-0.3,0.3); 
        
    case 'regular1'
        options.alpha = 1;
        M = nm-toolbox_signal.load_image('fnoise',n,options);
    case 'regular2'
        options.alpha = 2;
        M = nm-toolbox_signal.load_image('fnoise',n,options);
    case 'regular3'
    ...
    case 'sparsecurves'
        options.alpha = 3;
        M = nm-toolbox_signal.load_image('fnoise',n,options);
        M = nm-toolbox_general.rescale(M);
        ncurves = 3;
        M = cos(2*pi*ncurves);
        
    case 'geometrical'

        J = nm-toolbox_general.getoptions(options, 'Jgeometrical', 4);
        sgeom = 100*n/256;
        options.bound = 'per';
        A = ones(n);
        for j=0:J-1
            ...
            for k=1:2^j
                I = find(B==k);
                U = nm-toolbox_signal.perform_blurring(randn(n),sgeom,options);
                s = median(U(I));
                I1 = find( (B==k) & (U>s) );
                I2 = find( (B==k) & (U<=s) );
                A(I1) = 2*k-1;
    ...
    case 'lic-texture'
        ...
        options.sigma_tensor = nm-toolbox_general.getoptions(options, 'lic_regularity', 50*n/256);
        T = compute_tensor_field_random(n,options);
        Flow = nm-toolbox_signal.perform_tensor_decomp(T); % extract eigenfield.
        options.isoriented = 0; % no orientation in streamlines
        % initial texture
        lic_width = nm-toolbox_general.getoptions(options, 'lic_width', 0);
        M0 = nm-toolbox_signal.perform_blurring(randn(n),lic_width);
    ...
    case 'square_texture'
        M = nm-toolbox_signal.load_image('square',n);
        M = nm-toolbox_general.rescale(M);
    ...
    case 'tv-image'
        M = rand(n);
        tau = nm-toolbox_signal.compute_total_variation(M);        
        options.niter = 400;
    ...
    case 'line-windowed'
        ...
        [Y,X] = meshgrid(x,x);
        eta = .3; 
        gamma = nm-toolbox_general.getoptions(options, 'gamma', pi/10);
        parabola = nm-toolbox_general.getoptions(options, 'parabola', 0);
    ...
    case 'grating'
        x = linspace(-1,1,n);
        [Y,X] = meshgrid(x,x);
        theta = nm-toolbox_general.getoptions(options, 'theta', .2);
        freq = nm-toolbox_general.getoptions(options, 'freq', .2);
        X = cos(theta)*X + sin(theta)*Y;
        M = sin(2*pi*X/freq);
    ...
    case 'twodisks'
        ...
        M = M + nm-toolbox_signal.load_image('disk', n, options);
    case 'diskregular'
        M = nm-toolbox_general.rescale(nm-toolbox_signal.load_image('disk',n,options));
        if not(isfield(options, 'alpha'))
            options.alpha = 3;
        end
        S = nm-toolbox_signal.load_image('fnoise',n,options);
    ...
    case 'fnoise'
        ...
        %   |M^(omega)| = 1/f^{omega}
        alpha = nm-toolbox_general.getoptions(options, 'alpha', 1);
        M = gen_noisy_image(n,alpha);
        
        
    case 'gaussiannoise'
        % generate an image of filtered noise with gaussian
        sigma = nm-toolbox_general.getoptions(options, 'sigma', 10);
        M = randn(n);
        m = 51;
        h = nm-toolbox_signal.compute_gaussian_filter([m m],sigma/(4*n),[n n]);
        M = nm-toolbox_signal.perform_convolution(M,h);
        return;
    ...
    case 'parabola'
        ...
        % curvature
        c = nm-toolbox_general.getoptions(c, 'c', .1);
        % angle
        theta = nm-toolbox_general.getoptions(options, 'theta',  pi/sqrt(2));
        x = -0.5:1/(n-1):0.5;
        [Y,X] = meshgrid(x,x);
    ...
    case 'nm-toolbox_signal.phantom'
        
        M = nm-toolbox_signal.phantom(n);
        
    case 'periodic_bumps'
        nbr_periods = nm-toolbox_general.getoptions(options, 'nbr_periods', 8);
        theta = nm-toolbox_general.getoptions(options, 'theta', 1/sqrt(2));
        skew = nm-toolbox_general.getoptions(options, 'skew', 1/sqrt(2) );        
    ...
    case 'noise'
        sigma = nm-toolbox_general.getoptions(options, 'sigma', 1);
        M = randn(n) * sigma;
        ...
        for i=1:length(ext)
            ...
            if( exist(name) )
                ...
                if not(isempty(n)) && (n~=size(M, 1) || n~=size(M, 2)) && nargin>=2
                    M = nm-toolbox_signal.image_resize(M,n,n);
                end
                ...
                if sigma>0
                    M = nm-toolbox_signal.perform_blurring(M,sigma);
                end
                return;
            end
        end
