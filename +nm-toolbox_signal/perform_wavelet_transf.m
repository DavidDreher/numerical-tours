...
options.null = 0;
h = nm-toolbox_general.getoptions(options, 'filter', '9-7');
...
if isstr(h)
   ...
   switch lower(h)
       case 'haar'
           if d==1 || separable==0
               x = nm-toolbox_signal.perform_haar_transf(x, Jmin, dir, options);
               return;
           end
       case {'linear' '5-3'}
           h = [1/2 1/4 sqrt(2)];
...
ti = nm-toolbox_general.getoptions(options, 'ti', 0);
...
if d==2 && separable==1
    ...
    if dir==1
        for i=1:n
            x(:,i) = nm-toolbox_signal.perform_wavelet_transf(x(:,i),Jmin,dir,options);
        end
        for i=1:n
            x(i,:) = nm-toolbox_signal.perform_wavelet_transf(x(i,:)',Jmin,dir,options)';
        end
