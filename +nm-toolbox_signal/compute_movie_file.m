...
options.null = 0;
fps = nm-toolbox_general.getoptions(options, 'fps', 10);
format = nm-toolbox_general.getoptions(options, 'format', []);
...
imap = nm-toolbox_general.getoptions(options, 'colormap', jet(256));
loopCount = nm-toolbox_general.getoptions(options, 'loopcount', 1);
verb = nm-toolbox_general.getoptions(options, 'verb', 0);

for j=1:nb
    if verb
        nm-toolbox_general.progressbar(j,nb);
    end
    if iscell(A)
        B = A{j};
    else
        % A is a filename
        B = nm-toolbox_signal.load_image( [A '-' num2string_fixeddigit(j-1,3)] );
    end
    B = round(nm-toolbox_general.rescale( B, 1,256));
    switch format
        case 'avi'
            if size(B,3)==3
                frame = im2frame( nm-toolbox_general.rescale(B) );
            else
