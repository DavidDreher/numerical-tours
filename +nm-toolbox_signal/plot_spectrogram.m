...
if iscell(S)
    ...
    for i=1:length(S)
        ...
        nm-toolbox_signal.plot_spectrogram(S{i}, [], options);
    end
    return;
end

% keep only low frequencies.
S = nm-toolbox_general.rescale(abs(S(1:end/2,:)));

normalization = nm-toolbox_general.getoptions(options , 'normalization', 'log');
