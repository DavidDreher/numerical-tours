...
if iscell(x)
    for i=1:length(x)
        x{i} = nm-toolbox_signal.perform_hist_eq(x{i},y{i},options);
    end
    return;
end

absval  = nm-toolbox_general.getoptions(options, 'absval', 0);
cols    = nm-toolbox_general.getoptions(options, 'cols', 0);
rows    = nm-toolbox_general.getoptions(options, 'rows', 0);
dim3    = nm-toolbox_general.getoptions(options, 'dim3', 0);
lambda  = nm-toolbox_general.getoptions(options, 'histinterp', 1);
if cols && rows
