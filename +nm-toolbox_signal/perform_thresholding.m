...
if nargin==3 && isstruct(type)
    options = type; 
    type = nm-toolbox_general.getoptions(options, 'type', 'hard');
end
if nargin<3
    type = 'hard';
end
...
if iscell(x) && not(strcmp(type, 'freeze'))
    ...
    for i=1:size(x,1)
        for j=1:size(x,2)
            y{i,j} = nm-toolbox_signal.perform_thresholding(x{i,j},t, type);
        end
    end
    return;
end
...
switch lower(type)
    ...
    case {'block' 'block-hard' 'block-soft'}
        ...
        bs = nm-toolbox_general.getoptions(options, 'block_size', 4);
...
dm = nm-toolbox_general.nb_dims(x);
...
n = size(x,1);
if nm-toolbox_general.nb_dims(x)==2
    p = size(x,2);
    if length(bs)==1
        bs = [bs bs];
    end
...
elseif nm-toolbox_general.nb_dims(x)==1
    %% 1D %%
...
elseif nm-toolbox_general.nb_dims(x)==3
    for i=1:size(x,3)
