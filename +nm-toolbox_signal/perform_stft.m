...
if length(w)>1
    ...
    if ~iscell(x)
        % direct transform
        for i=1:length(w)
            y{i} = nm-toolbox_signal.perform_stft(x, w(i), q(i), options);
        end
    else
        n = nm-toolbox_general.getoptions(options, 'n', 1, 1);
        y = zeros(n,size(x{1}, 3));
        % backward transform
        for i=1:length(w)
            y = y + nm-toolbox_signal.perform_stft(x{i}, w(i), q(i), options);
        end
        y = y/length(w);
    end    
    return;    
...
multichannel = nm-toolbox_general.getoptions(options, 'multichannel', 0);

if multichannel
    options.multichannel = 0;
    % multichannel transform
    if nm-toolbox_general.nb_dims(x)==3
        % backward transform
        for i=1:size(x,3)
             y(:,i) = nm-toolbox_signal.perform_stft(x(:,:,i), w,q, options);
        end
    else
        for i=1:size(x,2)
             y(:,:,i) = nm-toolbox_signal.perform_stft(x(:,i), w,q, options);
...
bound = nm-toolbox_general.getoptions(options, 'bound', 'per');
transform_type = nm-toolbox_general.getoptions(options, 'transform_type', 'fourier');
normalization = nm-toolbox_general.getoptions(options, 'normalization', 'tightframe');
window_type = nm-toolbox_general.getoptions(options, 'window_type', 'sin');
