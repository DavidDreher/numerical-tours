...
%                'denoising_' 'inverse_' 'graphics_' 'multidim_' 'mesh_' 'variational_' 'fastmarching_'};
    list_ext = {'audio' 'coding' 'cs' 'denoising' 'fastmarching' 'graphics' 'introduction' 'inverse' 'mesh' 'multidim' 'numerics' 'sparsity' 'variational' 'wavelet' 'optim'};
    a = dir('*_*.m');
    for i=1:length(a)
        name = a(i).name;
        for k=1:length(list_ext)
            if not(isempty( findstr(name, list_ext{k}) ))
                ...
                nm-toolbox_publishing.perform_publishing(name(1:end-2));
            end
        end
    end
    % zip all toolbox files
    nm-toolbox_publishing.perform_toolbox_zipping();
    return;
end
...
if iscell(name)
    for i=1:length(name)
        ...
        nm-toolbox_publishing.perform_publishing(name{i}, options);
    end
    return;
end
...
if not(isempty(strfind(name, '*')))
    ...
    nm-toolbox_publishing.perform_publishing(name, options);
    return;
end

%% set up path
% directory where the wavelet-tour web site is
repweb = nm-toolbox_general.getoptions(options, 'rep', '../html/');
...
repprivate = nm-toolbox_general.getoptions(options, 'repprivate', 'private-svg/');
...
% open files
fid     = fopen([name '.m'], 'rt');
fidout  = fopen([repweb 'index.m'], 'wt');
if fid<0
...
%% do the publishing
opts.format = nm-toolbox_general.getoptions(options, 'format', 'html');
if not(strcmp(opts.format, 'latex'))
    opts.stylesheet = nm-toolbox_general.getoptions(options, 'stylesheet', [repweb 'nt.xsl']);
    opts.stylesheet = [pwd '/' opts.stylesheet];
end
filename = [repweb 'index.m'];
opts.outputDir = [pwd '/' rep];
% opts.format = 'xml';

path(repweb, path);
file = publish('index.m',opts);

% web(file);
% delete([repweb 'exo*.m']);
% delete([repweb 'index.m']);
...
nm-toolbox_publishing.perform_online_publishing(name);
