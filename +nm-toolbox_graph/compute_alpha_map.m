...
options.null = 0;
nalpha = nm-toolbox_general.getoptions(options, 'nalpha',256);
sigma = nm-toolbox_general.getoptions(options, 'sigma',.08);
c = nm-toolbox_general.getoptions(options, 'center',.4);
...
cname = nm-toolbox_general.getoptions(options, 'cname', 'jet');
% compute an associate colormap 
c0 = eval([cname '(256);']);
nc = size(c0,1);
si = nm-toolbox_general.rescale(cumsum(a));
s = linspace(0,1,nc);
c = zeros(length(si),3);
for i=1:3
