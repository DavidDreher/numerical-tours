...
verb = nm-toolbox_general.getoptions(options, 'verb', n>5000);
...
switch lower(type)
    case 'combinatorial'
        W = nm-toolbox_graph.triangulation2adjacency(face);
    case 'distance'
        W = my_euclidean_distance(nm-toolbox_graph.triangulation2adjacency(face),vertex);
        W(W>0) = 1./W(W>0);
        W = (W+W')/2; 
    case 'conformal'
        % conformal laplacian        
        ...
        for i = 1:n
            if verb
                nm-toolbox_general.progressbar(i,n);
            end
            for b = ring{i}
