...
options.null = 0;
display_flips = nm-toolbox_general.getoptions(options, 'display_flips', 0);
...
estack = find( nm-toolbox_graph.check_incircle_edge(vertex,face,edge)==0 );

A = sort( nm-toolbox_graph.compute_triangulation_angles(vertex,face) );
...
while not(isempty(estack))
    if verb
        ...
        nm-toolbox_general.progressbar( max(vbar,1), ntot);
    end
    count = count+1;
    if count>Inf % ntot
    ...
    % display
    if display_flips
        clf; nm-toolbox_graph.plot_graph(nm-toolbox_graph.triangulation2adjacency(face),vertex);
        drawnow; % pause(.01);
    end    
    
    Anew = sort( nm-toolbox_graph.compute_triangulation_angles(vertex,face) );
    if lexicmp(Anew,A)<=0
...
nm-toolbox_general.progressbar(ntot, ntot);
