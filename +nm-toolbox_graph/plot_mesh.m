...
name            = nm-toolbox_general.getoptions(options, 'name', '');
normal          = nm-toolbox_general.getoptions(options, 'normal', []);
face_color      = nm-toolbox_general.getoptions(options, 'face_color', .7);
edge_color      = nm-toolbox_general.getoptions(options, 'edge_color', 0);
normal_scaling  = nm-toolbox_general.getoptions(options, 'normal_scaling', .8);
sanity_check    = nm-toolbox_general.getoptions(options, 'sanity_check', 1);
view_param      = nm-toolbox_general.getoptions(options, 'view_param', []);
texture         = nm-toolbox_general.getoptions(options, 'texture', []);
texture_coords  = nm-toolbox_general.getoptions(options, 'texture_coords', []);
tmesh           = nm-toolbox_general.getoptions(options, 'tmesh', 0);

if size(vertex,1)==2
    % 2D triangulation
    % vertex = cat(1,vertex, zeros(1,size(vertex,2)));
    nm-toolbox_graph.plot_graph(nm-toolbox_graph.triangulation2adjacency(face),vertex);
    return;
end
...
if size(face,1)==4 && tmesh==1
    ...
    w = nm-toolbox_general.getoptions(options, 'cutting_plane', [0.2 0 1]');
    w = w(:)/sqrt(sum(w.^2));
    t = sum(vertex.*repmat(w,[1 size(vertex,2)]));
    a = nm-toolbox_general.getoptions(options, 'cutting_offs', median(t(:)) );
    b = nm-toolbox_general.getoptions(options, 'cutting_interactive', 0);
    plot_points = nm-toolbox_general.getoptions(options, 'plot_points', 0);
    ...
    while true;
        ...
        if not(isempty(facetrim))
            face1 = nm-toolbox_general.tet2tri(facetrim, vertex, 1);
            % options.method = 'slow';
            face1 = nm-toolbox_general.perform_faces_reorientation(vertex,face1, options);
            h{1} = nm-toolbox_graph.plot_mesh(vertex,face1, options);
        end
        view(3); % camlight;
        shading faceted;
        if plot_points
...
if ~isempty(normal)
    %%% plot the normals %%%
    n = size(vertex,1);
    subsample_normal = nm-toolbox_general.getoptions(options, 'subsample_normal', min(4000/n,1) );
