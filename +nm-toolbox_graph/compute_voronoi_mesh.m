...
options.end_points = [];
[D0,S,Q0] = nm-toolbox_graph.perform_fast_marching_mesh(vertex, faces, start_points, options);

D = [];
for i=1:nstart
    if verb
        nm-toolbox_general.progressbar(i,nstart);
    end
    options.dmax = max( D0(Q0==start_points(i)) )*1.1;
    [D(:,i),S,Q] = nm-toolbox_graph.perform_fast_marching_mesh(vertex, faces, start_points(i), options);
end
...
edge = nm-toolbox_graph.compute_edges(faces);
i = edge(1,:); j = edge(2,:);
% crossings
I = find(Q1(i)~=Q1(j));
i = i(I); j = j(I);
...
d = D( j' + (qj-1)*n );
lambda = (c-a) ./ (b-a+c-d);
lambda = nm-toolbox_general.clamp(lambda(:)',0,1);
% lambda = 1/2+lambda*0;
% point locations
