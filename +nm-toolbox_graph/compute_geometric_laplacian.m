...
if strcmp(lower(type),'combinatorial')
    L = nm-toolbox_graph.compute_combinatorial_laplacian( nm-toolbox_graph.triangulation2adjacency(face) );
    return;
end
if strcmp(lower(type),'distance')
    D = build_euclidean_weight_matrix(nm-toolbox_graph.triangulation2adjacency(face),vertex,0);
    D(D>0) = 1/D(D>0).^2;
    L = diag( sum(D) ) - D;
    L = (L+L')/2; 
    return;
...
ring = nm-toolbox_graph.compute_vertex_face_ring(face);
...
elseif strcmp(lower(type),'conformal') || strcmp(lower(type),'authalic')
    if nargin<4
        ...
        ring = nm-toolbox_graph.compute_vertex_ring( face );
    end
