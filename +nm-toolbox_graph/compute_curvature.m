...
options.null = 0;
naver = nm-toolbox_general.getoptions(options, 'curvature_smoothing', 3);
verb = nm-toolbox_general.getoptions(options, 'verb', 1);

[V,F] = nm-toolbox_graph.check_face_vertex(V,F);

n = size(V,2);
m = size(F,2);

% associate each edge to a pair of faces
Af = -nm-toolbox_graph.triangulation2adjacency(F);
...
% normals to faces
[tmp,normal] = nm-toolbox_graph.compute_normal(V,F);

% inner product of normals
dp = sum( normal(:,E(:,1)) .* normal(:,E(:,2)), 1 );
% angle un-signed
beta = acos(nm-toolbox_general.clamp(dp,-1,1));
% sign
...
for x=1:3
    for y=1:3
        a = Tv(x,y,:);
        a = nm-toolbox_graph.perform_mesh_smoothing(F,V,a(:),options);
...
normal = nm-toolbox_graph.compute_normal(V,F);
