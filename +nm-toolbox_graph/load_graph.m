...
if strcmp(name(end-2:end), 'off') || strcmp(name(end-2:end), 'ply') || strcmp(name(end-2:end), 'smf')
    ...
    [vertex,face] = nm-toolbox_graph.read_mesh(name);
    vertex = vertex(1:2,:);
    A = sparse( nm-toolbox_graph.triangulation2adjacency(face,vertex) );
    return;
end
...
switch lower(name)
    ...
    case 'rand_triangulation'
        ...
        A = nm-toolbox_graph.triangulation2adjacency(face,vertex);
...
%�iven the number of points, nodes are uniformly distributed
