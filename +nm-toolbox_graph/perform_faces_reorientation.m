function faces = nm-toolbox_general.perform_faces_reorientation(vertex,faces, options)
...
options.null = 0;
method = nm-toolbox_general.getoptions(options, 'method', 'fast');
verb = nm-toolbox_general.getoptions(options, 'verb', 1);

[vertex,faces] = nm-toolbox_graph.check_face_vertex(vertex,faces);
n = size(vertex,2);
m = size(faces,2);
...
options.method = 'fast';
faces = nm-toolbox_graph.nm-toolbox_general.perform_faces_reorientation(vertex,faces, options);

fring = nm-toolbox_graph.compute_face_ring(faces);
tag = zeros(m,1)-1;
heap = 1;
for i=1:m
    if m>100 & verb==1
        nm-toolbox_general.progressbar(i,m);
    end
    if isempty(heap)
        I = find(tag==-1); 
        if isempty(I)
...
[normal,normalf] = nm-toolbox_graph.compute_normal(vertex,faces);
a = mean(vertex,2);
a = repmat(a, [1 n]);
