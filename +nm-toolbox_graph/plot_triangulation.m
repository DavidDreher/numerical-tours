...
options.null = 0;
lw = nm-toolbox_general.getoptions(options, 'edgewidth', 2);
ms = nm-toolbox_general.getoptions(options, 'vertexsize', 20);

edges = nm-toolbox_graph.compute_edges(faces);

vertex = vertex(2:-1:1,:);

if not(isempty(M))
    nm-toolbox_signal.imageplot(M);
end
hold on;
