...
options.null = 0;
method = nm-toolbox_general.getoptions(options, 'method', 'continuous');
...
if size(x,1)>1 && size(x,2)>1
    ...
    path = {};
    for i=1:size(x,2)
        path{end+1} = nm-toolbox_graph.compute_geodesic(D, x(:,i), options);
    end
    return;
end
...
J = find(A~=Inf);
A1 = A; A1(I) = mmax(A(J));
global nm-toolbox_signal.grad;
nm-toolbox_signal.grad = compute_grad(A1);
nm-toolbox_signal.grad = -perform_vf_normalization(nm-toolbox_signal.grad);

% path extraction
path = stream2(nm-toolbox_signal.grad(:,:,2),nm-toolbox_signal.grad(:,:,1),end_points(2,:),end_points(1,:), str_options);
for i=1:length(path)
    path{i} = path{i}(:,2:-1:1);
end
if length(path)==1
...
global nm-toolbox_signal.grad;
gx = nm-toolbox_signal.grad(:,:,1);
gy = nm-toolbox_signal.grad(:,:,2);
...
p = nm-toolbox_general.clamp(p,1,n-1);
q = nm-toolbox_general.clamp(q,1,n-1);

% residual in [0,1]
xx = y(1)-p;
yy = y(2)-q;

xx = nm-toolbox_general.clamp(xx,0,1);
yy = nm-toolbox_general.clamp(yy,0,1);
...
function nm-toolbox_signal.grad = compute_grad(M,options)
...
if type==1
    ...
    D1 = [M(2:end,:);M(end,:)];
    D2 = [M(1,:);M(1:end-1,:)];
    nm-toolbox_signal.grad(:,:,1) = (D1-D2)/(2*h1); 
    nm-toolbox_signal.grad(1,:,1) = ( 4*M(2,:) - 3*M(1,:) - M(3,:) )/(2*h1);
    nm-toolbox_signal.grad(end,:,1) = -( 4*M(end-1,:) - 3*M(end,:) - M(end-2,:) )/(2*h1);
    % central differences on Y
    D1 = [M(:,2:end),M(:,end)];
    D2 = [M(:,1),M(:,1:end-1)];
    nm-toolbox_signal.grad(:,:,2) = (D1-D2)/(2*h2); 
    nm-toolbox_signal.grad(:,1,2) = ( 4*M(:,2) - 3*M(:,1) - M(:,3) )/(2*h2);
    nm-toolbox_signal.grad(:,end,2) = -( 4*M(:,end-1) - 3*M(:,end) - M(:,end-2) )/(2*h2);
elseif type==2
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % accumulate on Y
    ...
    D1 = [MM(2:end,:);MM(end,:)];
    D2 = MM;
    nm-toolbox_signal.grad(:,:,1) = (D1-D2)/h1;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % accumulate on X
    MM = ( M + [M(2:end,:);M(end,:)] )/2;
    ...
    D1 = [MM(:,2:end),MM(:,end)];
    D2 = MM;
    nm-toolbox_signal.grad(:,:,2) = (D1-D2)/h2;
elseif type==3
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % accumulate on Y
    ...
    D1 = MM;
    D2 = [MM(1,:);MM(1:end-1,:)];
    nm-toolbox_signal.grad(:,:,1) = (D1-D2)/h;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % accumulate on Y
    MM = ( M + [M(1,:);M(1:end-1,:)] )/2;
    % fwd differences on Y
    D1 = MM;
    D2 = [MM(:,1),MM(:,1:end-1)];
    nm-toolbox_signal.grad(:,:,2) = (D1-D2)/h;
else
...
if type~=1
    ...
    for j=1:p-1
        h(:,j,1) = ( nm-toolbox_signal.grad(:,j,1)+nm-toolbox_signal.grad(:,j+1,1) )/2;
    end
    for i=1:n-1
        h(i,:,2) = ( nm-toolbox_signal.grad(i,:,2)+nm-toolbox_signal.grad(i+1,:,2) )/2;
    end
    
    if type==2          % fwd differences
        nm-toolbox_signal.grad(1:n-1,1:p-1,:) = h(1:n-1,1:p-1,:);        
    elseif type==3      % bwd differences
        nm-toolbox_signal.grad(2:n,2:p,:) = h(2:n,2:p,:);        
    end   
