...
if length(x)>1
    ...
    for i=1:length(x)
        if length(x)>5     
            if verb
                nm-toolbox_general.progressbar(i,length(x));  
            end
        end
        [path{i},vlist{i},plist{i}] = nm-toolbox_graph.compute_geodesic_mesh(D, vertex, face, x(i), options);
    end
    return;
end
...
if strcmp(method, 'discrete')
    if isfield(options, 'v2v')
        ...
        vring =  nm-toolbox_graph.compute_vertex_ring(face);
    end
...
if isfield(options, 'v2v')
    ...
    v2v = nm-toolbox_graph.compute_vertex_ring(face);
end
