...
options.niter_averaging = nm-toolbox_general.getoptions(options, 'covariance_smoothing', 10);

C = zeros(d,d,n);
for i=1:d
    for j=1:i
        ai = nm-toolbox_graph.perform_mesh_smoothing(face,vertex,f(i,:)',options);
        aj = nm-toolbox_graph.perform_mesh_smoothing(face,vertex,f(j,:)',options);
        aij = nm-toolbox_graph.perform_mesh_smoothing(face,vertex, f(i,:)' .* f(j,:)',options);
...
if nargout>1
    ...
    D = zeros(d,n);
    for k=1:n
        nm-toolbox_general.progressbar(k,n);
        [u,d] = eig(C(:,:,k));
        d = real(diag(d));
    ...
    normal = nm-toolbox_graph.compute_normal(vertex,face);
    Normal = squeeze(U(:,1,:));
