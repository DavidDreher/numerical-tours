...
start_points = nm-toolbox_general.getoptions(options, 'start_points', []);
ve = nm-toolbox_general.getoptions(options, 'voronoi_edges', []);
ms = nm-toolbox_general.getoptions(options, 'ms', 20);
lw = nm-toolbox_general.getoptions(options, 'lw', 3);

colorfx = nm-toolbox_general.getoptions(options, 'colorfx', 'none');
if strcmp(colorfx, 'wrapping')
    num = 20;
    col = nm-toolbox_general.rescale(col);
    col = mod(col*num,2); col(col>1) = 2-col(col>1);
end
if strcmp(colorfx, 'equalize')    
    col(col>0) = nm-toolbox_signal.perform_hist_eq(col(col>0), 'linear');
end

options.face_vertex_color = col;
% clf;
hold on;
nm-toolbox_graph.plot_mesh(vertex, faces, options);
