...
end_points = nm-toolbox_general.getoptions(options, 'end_points', []);
verbose = nm-toolbox_general.getoptions(options, 'verbose', 1);
nb_iter_max = nm-toolbox_general.getoptions(options, 'nb_iter_max', Inf);
values = nm-toolbox_general.getoptions(options, 'values', []);
L = nm-toolbox_general.getoptions(options, 'constraint_map', []);
H = nm-toolbox_general.getoptions(options, 'heuristic', []);
dmax = nm-toolbox_general.getoptions(options, 'dmax', Inf);

d = nm-toolbox_general.nb_dims(W);
...
if d==2
    if exist('nm-toolbox_graph.perform_front_propagation_2d')~=0
        [D,S,Q] = nm-toolbox_graph.perform_front_propagation_2d(W,start_points-1,end_points-1,nb_iter_max, H, L, values);
    else
...
elseif d==3
    [D,S,Q] = nm-toolbox_graph.perform_front_propagation_3d(W,start_points-1,end_points-1,nb_iter_max, H, L, values);
end
Q = Q+1;
