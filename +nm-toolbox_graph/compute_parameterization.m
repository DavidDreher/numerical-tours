...
options.null = 0;
method      = nm-toolbox_general.getoptions(options, 'method', 'parameterization');
laplacian   = nm-toolbox_general.getoptions(options, 'laplacian', 'conformal');
boundary    = nm-toolbox_general.getoptions(options, 'boundary', 'square');
ndim        = nm-toolbox_general.getoptions(options, 'ndim', 2);
...
switch lower(method)
    ...
    case 'drawing'
        ...
        L = nm-toolbox_graph.compute_mesh_laplacian(vertex,face,'combinatorial',options);
    ...
    case 'flattening' 
    	L = nm-toolbox_graph.compute_mesh_laplacian(vertex,face,laplacian,options);
        [U,S] = eig(full(L));
        vertex1 = U(:,2:3);
        if 0
            [U,S,V] = eigs(L, ndim+1, 'SM');
    ...
    case 'isomap'
        A = nm-toolbox_graph.triangulation2adjacency(face);
...
% extract boundary
boundary = nm-toolbox_graph.compute_boundary(face,options);
